...
* **`company`** presentaba inicialmente un **94% de valores nulos**, lo que pudiera parecer una falta de información que podría impedir extraer información fiable. Sn embargo podemos sustituir estos valores nulos por un valor que indique que la reserva pertenece a un cliente particular (por ejemplo, -1) que es lo que semanticamente representan estos nulos, no una falta de información. A pesar de haber realizado esta sustitución, esta variable finalmente fue eliminada del conjunto de features ya que al explorar el modelo, esta variable no aportaba cambios significativos en los resultados.
